...
- case4：可供审阅的 Pencil 设计源已放入 `03-design/case4/case4.pen`（帧见 `doc/case4/DESIGN-NOTES.md`）。**未**冻结 Gate 1。首版仅 2D。更新日期：2026-09-13。
...
- case4 最终轨迹门槛（2026-09-15 用户确认）：与 case3 一致，不要求实际点数等于 base；预期 38 点、三方案均收齐 30 点可正常完成，前提是有完整点、数据合法、无未完成尾部且最终统计四文件通过。不补造缺点；结果不齐或读取失败沿用累计失败 10 次回退机制。
- case4 控制遗留（2026-09-13）：单次启动覆盖三方案时的 dt_type 已由用户澄清为 "with dt"；reinit 同为 "with dt"，init 为 ""，均已确认，详见后端问题清单。
- case4 CEP 百分比（2026-09-15 用户确认）：DT 相对传统基站，分别计算 CEP50/90 变化，预期降低但按实际数据展示；Cursor 需同步设计。用户已说明截图仅示意，数值与方案映射按接口说明及文件，不照抄截图，该疑问已关闭。
- case4 控制状态（用户确认）：沿用 case2/case3 的 init/start/reinit 和 execute success/execute fail/case complete/reinit complete；start 的 dt_type 已确认精确值为 "with dt"；reinit 同为 "with dt"，init 为 ""，均已确认。
- case4 现场环境（用户确认）：入口和弹窗机制、内容、界面与当前 case3 保持一致。
- case4 吞吐率对齐遗留：2026-09-15 后端答复明确有/无 DT 各自有数据就更新，不互相收齐。
...
- 当前焦点：case4 Pencil 设计源供用户视觉审阅；未经用户确认不得标记 Gate 1 冻结。case2/case3 已有验收口径保持不变，真实环境 QA 仍待外部条件。
...
case4：Pencil 设计源完成后等待用户视觉审阅；四项后端问题已于 2026-09-15 核对澄清，结论见 doc/case4/BACKEND-QUESTIONS.md；不得沿用旧的待确认或“不画无效点”口径。未冻结前不进入静态 HTML / 契约施工。
