...
- case4 2026-09-17 增量：用户确认真实后端、共享挂载、真实采集验证 PASS（用户提供，非本轮独立复验）；当前为功能增强阶段。逐点误差统一 XYZ，现有公式已支持，本轮仅同步说明。Start/ReInit 已硬切换 `dt_type="all"`，init 为空，拒绝旧值；用户确认验证并上库，提交 `5067ffd`、`831984a`。CEP 百分比已实现并获用户接受，提交 `c4382a3`；用户报告已推 main、Issue #2 已关闭。包含上升/下降方向及DT数值避让，条件/精度/位置见 `doc/case4/WEB-SPEC.md`；Reflection/T4 继续保留。下方带日期的旧阶段记录仅供追溯，冲突以本条为准。
