...
- case4 CEP 百分比（2026-09-15 用户确认）：DT 相对传统基站，分别计算 CEP50/90 变化，预期降低但按实际数据展示；该百分比已由用户决定后置，首版不做。用户已说明截图仅示意，数值与方案映射按接口说明及文件，不照抄截图，该疑问已关闭。
...
- case4 完成态交互（已后置，首版不实现）：测试完成后悬停误差回溯区 Pi，弹出三种方案该点具体误差（本轮 XY 派生值）；测试中不启用。
...
- 当前焦点：case4 主线 2D Pencil 已由用户确认冻结，下一阶段为首版静态验收；百分比和悬停后置。case2/case3 已有验收口径保持不变，真实环境 QA 仍待外部条件。
...
case4：Pencil 主线 2D 已由用户确认冻结，进入首版静态验收准备；四项后端问题已于 2026-09-15 核对澄清，结论见 doc/case4/BACKEND-QUESTIONS.md；不得沿用旧的待确认或“不画无效点”口径。静态/正式实现均不纳入已后置的百分比与悬停；正式契约与 SPEC 仍按流程定稿。
