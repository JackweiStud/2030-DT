...
- case4：范围与 UX 三态已记录，正在 Gate 0/1 准备；Pencil 由用户安排 Cursor 完成，未记录设计冻结、静态验收或接口批准。首版仅 2D，单次开始→测试完成→重置；吞吐率与轨迹独立。业务依据见 `doc/case4/PHASE0-SCOPE.md`、`UX-STATE-MAP.md`、`INPUT-AND-OPEN-QUESTIONS.md`（后两者同在 `doc/case4/`）。更新日期：2026-09-13；用户已确认通过轨迹与误差证明 DT 辅助定位更准确，同时展示吞吐率与 NLOS。当前按每轮一个问题继续澄清，尚未充分明确需求，不输出具体施工计划。
...
- case3 换皮 Gate 1.5（v2）：2026-08-24 用户已人工视觉接受 `web-static/case3-v2/case3-v2.html` 工程底稿；`index.html` 仍为 Pencil 像素基准。记录见 `doc/case3/STATIC-HTML-ACCEPTANCE-v2.md`。P4 可按分区/令牌/切图做 React 新皮（先挂 case5），仍不得直接导入静态 CSS/JS 或假状态逻辑。旧皮 `web-static/case3/` 验收不撤销。
- case3 V2 功能验收：2026-08-25 用户确认 Ticket 01–07 全部人工验收完成，B 阶段到此停止；旧 `DT for Comm` 与临时第五 Tab 均暂不替换/删除。用户明确豁免 Ticket 07 新增三进程自动化，因此功能验收已完成，但可复放的 Case3 Web + Node + stub 三进程 E2E 证据仍未补齐；真实后端、真实挂载和真实采集仍不在完成口径内。
...
- 2026-08-13 BF-005：Case2 Initial 六文件失败时徽标改为「case2初始化数据异常」（不再显示「等待启动测试」）；启动仍禁用；不为此开探活。刷新或切 Tab 后才会再拉 Initial。
- case4 最终轨迹遗留（2026-09-13）：完成时三方案已相互收齐但点数少于 base，能否正常完成由用户次日找后端确认；不预设等长硬门槛或允许不足通过。
- case4 控制遗留（2026-09-13）：单次启动覆盖三方案时的 dt_type 尚未约定，用户计划次日找后端团队确认；不预设字段值，与 65535 无效坐标处理共同登记于接口遗留文档。
- case4 CEP 增减百分比（用户暂缓）：后续开发阶段再考虑，比较基准/公式/展示方式未冻结；CEP50/90 数值展示范围不变。
- case4 控制状态（用户确认）：沿用 case2/case3 的 init/start/reinit 和 execute success/execute fail/case complete/reinit complete；dt_type 仍待后端确认，不预设值。
- case4 现场环境（用户确认）：入口和弹窗机制、内容、界面与当前 case3 保持一致。
- case4 吞吐率对齐遗留：有/无 DT 样本是否同序号收齐后一起显示，由用户明天找后端确认，暂不默认独立展示或同步等待。
- case4 吞吐率横轴（用户确认）：显示“1、2、3…样点”，按吞吐采样序号，不换算秒数、不绑定地图 Pi。
- case4 实时文件发布（用户确认）：三方案定位轨迹及有/无 DT 吞吐率文件均 append 新行；三方案轨迹按行收齐，吞吐独立，用户确认开始/重置前由 Node 清空 case4 本轮动态文件，保留预期轨迹、地图和原始参考样本；半行处理及具体清理清单待契约明确。
- case4 新增完成态交互（用户确认）：测试完成后悬停误差回溯区 Pi，弹出三种方案该点具体误差（本轮 XY 派生值）；测试中不启用。
...
- case4 逐点偏差（2026-09-13 用户确认）：base 为预期轨迹；2D 按 XY 欧氏距离计算，后续 3D 加入 Z。三种定位结果均按文件行号与 base 配对，第 i 行对应 Pi；用户已确认同一行三方案收齐后同步展示轨迹及误差，按连续完整点位推进，吞吐独立；无效坐标 65535 的处理列为遗留，由用户后续找后端确认，暂不冻结处理行为；底部 CDF/CEP 保持后端统计来源。
...
- 当前焦点：case4 范围、UX 状态及接口差异梳理；Cursor 负责 Pencil，用户负责设计冻结。case2/case3 已有验收口径保持不变，真实环境 QA 仍待外部条件。
...
- [case4 范围与主线](doc/case4/PHASE0-SCOPE.md)
- [case4 UX 状态映射](doc/case4/UX-STATE-MAP.md)
- [case4 接口输入与待确认项](doc/case4/INPUT-AND-OPEN-QUESTIONS.md)
...
case4 当前推进方式：遵照用户要求，每轮只问一个问题，根据回答继续追问，达到充分理解真实目标与需求后再提出具体行动和计划。当前重要缺口包括本次交付的数据来源与验收边界、样本一致性、逐点误差口径和文件时序；不将这些缺口提前写成冻结方案。Pencil 仍由用户安排 Cursor 负责，未验收阶段不标记完成。
