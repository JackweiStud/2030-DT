...
- case3 旧皮退役（2026-09-23）：正式导航「DT辅助通信」仅挂 Case3 V2（`tab=case3`）；已删除旧 `Case3Page` / `case3.css` / 旧组件、`assets/case3`、`web-static/case3`、旧皮单测与 `e2e/case3-mainline`。业务脑仍在 `src/cases/case3/`（api/hooks/state/metrics/presentation/config）；临时 `case5` Tab id 已合回。设计源 `03-design/case3/` 与 Gate 1.5 验收记录保留为历史证据。
