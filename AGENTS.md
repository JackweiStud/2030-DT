...
- 当前已完成本地开发的活跃业务 Tab 为 `case2`（`DT Calibration`）与 `case3`（`DT for Comm`）；case1/case4 仍显示“建设中”，不加载业务状态或数据。
...
- case2/case3 的真实后端、真实挂载路径与真实采集数据均仍未验收。

## 共享与隔离

- Shell 只拥有：顶部导航、当前 Tab、1920×1080 缩放、公共视觉 token 和“建设中”占位页。
- case 业务状态、文件字段、指标语义、算法、媒体、弹层和业务 CSS 必须 case-local。
- 默认单活跃：切离 case2/case3 时清理对应 case 的轮询、临时数据和本地状态；不允许隐藏页继续读共享文件。任一业务 case Start/ReInit 等待期间锁定其他 Tab。
