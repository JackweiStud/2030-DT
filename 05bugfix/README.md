...
| ID                                                        | 严重度 | 区域              | 标题                                 | 状态     |
| --------------------------------------------------------- | --- | --------------- | ---------------------------------- | ------ |
| [BF-024](BF-024-screenshot-flag-edge-success-complete.md) | P0  | web/case2+case3 | 截图只跟 flag 0→1；左边界立刻拍测试中，右边界拍已渲染完成态 | ✅ done |
| [BF-025](BF-025-case2-value-range-clamp.md)               | P0  | server/case2    | 真实算法数据撞上统一范围；按指标可配，热力掐位、KPI 丢弃     | ✅ done |


BF-024：客户划定截图时机：success 左边界 0→1 立刻拍测试中；complete 右边界（含同拍）拍已渲染完成态（Case2 先六文件，Case3 先完成态渲染）。已落地。

BF-025：真实算法数据不再因统一 `\-200～200` / `0～500` 整批 422。范围按指标可配（`code/server/.env`）；热力越界双边掐位，KPI 越界丢弃样本。已落地。
