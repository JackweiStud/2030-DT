...
- Canvas：1920×1080
- Quality：标准重建 → `APPROVED`（2026-08-09 用户确认冻结）
...
| 更新 | Tab 激活为 DT for Comm；正式 UI 文案 `开销(%)`；动态占位符 |
...
| `case3.with-running` | Without 保留完成；With「测试中」；`reflection` 参与完整点校验但 v1 不渲染；BA 仍基线 |
...
│       │   ├── MapStage (map image, route, UE, BS; v1 no Reflection/LOS)
│       │   ├── BeamCard
│       │   └── PointProgressWindow (20 slots)
└── KpiComparePanel
    ├── CostCard
    ├── ThroughputCard
    └── BeamAccuracyCard
...
- 点位进度：20 槽横排；标题固定为「点位进度」，20 是最近窗口长度而非总点位上限。
...
|---|---|---|---|---|
| initial | Start on / Reset disabled | Start **disabled** / Reset disabled | 仅预置路线 | Cost/Thrp 空；BA 基线 |
| without-running | Start disabled / Reset disabled | 全 disabled | scan+sel+progress | without Cost/Thrp 更新中 |
| without-completed | Start disabled / Reset on | Start **on** / Reset disabled | without 终态保留 | without 值保留 |
| with-running | 全 disabled（互斥） | Start disabled / Reset disabled | with 轨迹与波束；v1 不渲染 Reflection/LOS | with 更新中；BA 基线 |
| with-completed | Start disabled / Reset on | Start disabled / Reset on | 双侧保留 | 全量对比 + BA 增量 |
