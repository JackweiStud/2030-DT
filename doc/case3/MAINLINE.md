...
| Without 完成 | 本轮已见 `execute success -> case complete`；最终快照满足 `ok=true,pendingTail=false,points>0,costPct!=null`，Web 渲染后写回 `init` | 停止轮询，保留 without 完成结果 | With Start 可用 | without 曲线/表盘保留；Beam Accuracy 仍为基线 |
...
| With 完成 | 本轮已见 `execute success -> case complete`；最终快照通过同一完整性门槛，Web 渲染后写回 `init` | 保留 without 完成结果 | 停止轮询，保留 with 完成结果 | Cost/Throughput 双侧对比；Beam Accuracy = 文件基线 + 当前配对点位增量 |
...
- 后端必须让 `execute success` 保持至少 3000ms；不得在 1000ms Web 轮询可能完全漏过的窗口内直接覆盖为完成终态。
