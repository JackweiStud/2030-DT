...
实现期可以从 `01-参考资料/case3/data/c3/` 一次性复制并校验 base route、BA baseline 和两侧逐点样本到 stub 自有 fixtures；运行时禁止回读 `01-参考资料/`。
...
- Without Cost：`25`
- With Cost：`15`
- 因而静态/E2E 可断言相对开销变化 `40.0%`
- Beam Accuracy baseline 可使用参考 `222,235`

参考目录原始 Cost 为 Without=`10`、With=`5`；不得把它们误复制进 stub fixtures 后仍断言 40.0%。Start/逐点发布日志标记 `dataSource:"reference-derived+local-demo-override"`；seed 日志仅用 `reference-derived`（见 §4）。这些值只用于本地演示和测试，不改变 API 契约或真实后端数据要求。
...
正式环境变量严格校验：共享根必须为绝对路径；poll/point/dwell 必须为十进制整数且 poll/point 为正、dwell 不低于 3000；outcome/log level 必须命中枚举，requestPicture/seedInit 只能为 0/1。非法配置快速退出并记录 `CONFIG_INVALID`，禁止 clamp。测试通过构造函数依赖注入更小 dwell/point，不通过正式环境变量绕过约束。
...
该动作仅用于本地无真实后端环境。真实挂载联调必须关闭 seed，使用真实后端提供的初始化文件。Start 逐点/Cost 发布日志使用 `dataSource:"reference-derived+local-demo-override"`。
...
- 与点数解耦；
- 至少在最终 complete 前写入一个有效值；
- 默认第一点完成后写该侧 fixture Cost，使运行中 UI 可显示；
...
| 精确 `case3 + start + dt_type + execute success` | 新建日志 `operationId,recovery:true`；清空该侧 4 个逐点文件和 Cost，从 fixture 第 1 点完整重放；禁止从现有 `K+1` 续写。全部关闭后按当前截图配置写 complete；若控制中 flag 已为 1，最终写必须保留高电平。 |
...
恢复沿用当前进程 `CASE3_STUB_REQUEST_PICTURE`；`CASE3_STUB_OUTCOME=fail` 不得把已经处于 `execute success` 的旧轮改写为 fail。恢复过程中仍按 §3.4 在每次清空、append、等待和终态 patch 前复验 ownership。
...
- 正式默认断言：poll=200ms、point=1000ms、dwell=3000ms、requestPicture=1、seedInit=1。
- 正式环境 dwell<3000、非正 poll/point、非法 outcome/flag/log level 均 `CONFIG_INVALID`，不 clamp；测试构造注入可使用更短间隔。
...
- 参考原始 Cost 10/5 不复制；本地 override 25/15 导出 40.0% 的 E2E 预期。
