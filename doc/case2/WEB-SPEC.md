...
**哨兵 `-1`：** 某张 Initial 或 Calibrated 矩阵中**只要出现** `-1`，该张**不画热力叠层**（与无矩阵相同，只留底图空槽）。其余指标/另一侧卡片不受牵连。不得把 `-1` 当最低误差着色。
...
| 无效哨兵 `-1` | **不进入** CDF / 均值 / 降幅的 `N` 与求和。图上 x 域与柱高只用有效样本，因此无效值不会以 `-1` 出现，轴从有效最小值起。某项 Initial 与（若展示）Calibrated 的有效样本都为空时，**不渲染**该行 CDF 与平均误差图（保留指标标题）。 |
| 下限 | 适配服务保证每条 KPI ≥1 个有限数；空/非法整批失败，Web 假定到达绘制时原始数组 `N≥1`。有效样本滤完后该项可以无图。 |
| 文件数值（适配已归一） | 全部 txt：语义精度 **2** 位小数（超过则适配**四舍五入**到 2 位后下发）。范围按指标可配：热力越界已掐位，KPI 越界已丢弃；热力矩阵**不**进入本节 CDF/均值。 |
...
对一组有效样本 `validSamples`（已去掉哨兵 `-1`，长度 `N≥1`）建台阶（该项有效样本为空则见 §9.0，不画该行图）：
...
- KPI 哨兵 `-1` 不得进入 CDF / 均值 / 降幅；不得把 `-1` 画进 x 域。该项两侧有效样本都空时不得画 CDF/柱图。
