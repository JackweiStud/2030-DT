...
- 取值范围按指标（归一后；热力越界由适配**双边掐位**；KPI 低于下限由适配**钳为 min 并保留**、高于上限**丢弃**，记越界个数）：
  - RSS 热力 **-1～500**；RSS KPI **-1～1000**
  - 有效路径数热力 **-1～500**；有效路径数 KPI **-1～50000**
  - 首径时延热力 **-1～1000**；首径时延 KPI **-1～10000**
  - 当前 KPI 下限均为 `-1`，低侧钳位结果由 Web 作为哨兵排除于 CDF、均值和降幅；RSS KPI 在 `[-1,0)` 内仍允许有效负误差值。
...
- 数值宜最多保留 **2** 位小数；超过 2 位时前端适配服务会**四舍五入**到 2 位后使用（不因此拒读）。
- 取值范围按指标（归一后；当前默认范围）：
  - RSS KPI **-1～1000**（`[-1,0)` 内允许有效负误差）
  - 有效路径数 KPI **-1～50000**
  - 首径时延 KPI **-1～10000**
- 低于下限的样本由适配钳为 `-1` 并保留，高于上限的样本丢弃并记越界数；Web 将 `-1` 排除于统计。高侧样本丢弃后仍须至少保留 1 个样本，否则前端侧整批失败。
...
- [ ] KPI 数值宜最多 2 位小数；当前默认范围按指标（RSS -1～1000、有效路径数 -1～50000、首径时延 -1～10000）；超过 2 位由适配四舍五入，低于下限钳为 `-1` 并保留，高于上限丢弃；Web 从统计中排除 `-1`。
