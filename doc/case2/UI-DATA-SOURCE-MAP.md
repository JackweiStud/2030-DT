...
| 截图保存反馈 | 是 | `save_picture_flag` 消费进度 | 启动路径：`execute success` 后至 `case complete`（含同拍）后端 0→1；Web 仅 `calibrating` 观察 | 同拍 complete 仍截一次；重置不置 1；适配服务落盘后清零，不覆盖。 |
...
|---|---|---|---|
| 热力图 | 正式契约为动态 `Nx × Ny` 非空矩形矩阵，语义精度 2 位小数（超过则适配四舍五入）；当前默认范围 RSS -1～500、有效路径数 -1～500、首径时延 -1～1000，越界由适配双边掐位；当前参考样本为 20×20 | 排除哨兵 `-1` 计算有效 min/max；所属矩阵格为 `-1` 时整格透明硬切，有效格内插值屏蔽无效邻点并重归一；覆盖到运行时底图 | Initial 读取失败：基线不可用；Calibrated 读取失败：整批拒绝。 |
| KPI | 正式契约为动态 `N` 个有限样本，语义精度 2 位小数（超过则适配四舍五入）；当前默认下限 `-1`：低于下限钳为 `-1` 并保留，高于上限丢弃；当前参考样本为 20×1 | 展平、去掉哨兵 `-1` 后排序、经验 CDF（点数随有效 `N`，过大才封顶下采样）、均值、降幅；该项有效样本为空时保留图表网格与坐标轴，不画曲线、柱和均值 | Initial 读取失败：该指标基线不可用；Calibrated 读取失败：整批拒绝。 |
| 降幅 | 无独立文件 | `(Calibrated 均值 - Initial 均值) / Initial 均值 × 100%`（相对增减）；箭头升/降；Initial 均值非正或无效：显示不可计算 |
