...
|---|---|---|---|
| 热力图 | 正式契约为动态 `Nx × Ny` 非空矩形矩阵，语义精度 2 位小数（超过则适配四舍五入）；范围按指标可配，越界由适配掐位；当前参考样本为 20×20 | 插值、配色、马赛克叠加到运行时底图；矩阵含哨兵 `-1` 则该张不画热力 | Initial 读取失败：基线不可用；Calibrated 读取失败：整批拒绝。 |
| KPI | 正式契约为动态 `N` 个有限样本，语义精度 2 位小数（超过则适配四舍五入）；范围按指标可配，越界样本由适配丢弃；当前参考样本为 20×1 | 展平、去掉哨兵 `-1` 后排序、经验 CDF（点数随有效 `N`，过大才封顶下采样）、均值、降幅；该项有效样本为空则不画 CDF/柱图 | Initial 读取失败：该指标基线不可用；Calibrated 读取失败：整批拒绝。 |
