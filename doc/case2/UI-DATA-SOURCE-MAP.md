...
>
> 对应契约：[doc/API-CONTRACT.md](../API-CONTRACT.md)。
...
| “启动 / 重置”按钮 | 是 | Web 本地交互 + 适配服务提交结果 | 当前状态可执行时 | “重置”唯一映射 `command=reinit`。 |
| 状态反馈、按钮禁用/运行中文字 | 是 | `command`、`status` 与 Web 本地提交态 | 状态机映射成立 | 适配服务断连不是 `execute fail`。 |
| Initial 三张热力图 | 是 | Initial 三个 20×20 文件 + 前端热图渲染 | Initial 输入通过校验 | 参考输入，不是本次校准成果。 |
| Calibrated 三张热力图 | 是 | Calibrated 三个 20×20 文件 + 前端热图渲染 | `case complete` 且六文件批次有效 | 不得显示静态 `heatmap-calibrated-represent.png`。 |
| 三张 Initial CDF、均值柱 | 是 | Initial 三组 20 个 KPI 样本 + 前端派生 | Initial KPI 输入通过校验 | CDF 为经验 CDF，均值为算术平均。 |
| 三张 Calibrated CDF、均值柱、降幅徽章 | 是 | Calibrated 三组 KPI 样本 + 前端派生 | `case complete` 且六文件批次有效 | 降幅运行时计算，不能沿用 40%/50% 视觉样例。 |
...
|---|---|---|---|
| 热力图 | 每文件 20×20 有限数值 | 插值、配色、马赛克叠加到运行时底图 | Initial 读取失败：基线不可用；Calibrated 读取失败：整批拒绝。 |
| KPI | 每文件 20 个有限数值；参考实物为 20×1 | 展平、排序、51 点经验 CDF、均值、降幅 | Initial 读取失败：该指标基线不可用；Calibrated 读取失败：整批拒绝。 |
| 降幅 | 无独立文件 | `(Initial 均值 - Calibrated 均值) / Initial 均值 × 100%` | Initial 均值非正或无效：显示不可计算，不用固定占位。 |

KPI 说明资料中的 4×5 只是排版示例；正式接口只认 20 个样本的总数和数值有效性。
...
- [x] 状态、重置、失败、刷新、Tab 切离、截图请求都有权威来源或明确待确认项。
- [x] Shell、静态资源、参考文件与正式运行输入已分层。
- [ ] P0-1 的批次/新鲜度机制确认后，补充“适配服务返回的已验证批次”证据字段。
