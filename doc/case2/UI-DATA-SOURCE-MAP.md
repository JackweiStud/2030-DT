...
|---|---|---|---|
| 热力图 | 正式契约为动态 `Nx × Ny` 非空矩形矩阵；当前参考样本为 20×20 | 插值、配色、马赛克叠加到运行时底图 | Initial 读取失败：基线不可用；Calibrated 读取失败：整批拒绝。 |
| KPI | 正式契约为动态 `N` 个有限样本；当前参考样本为 20×1 | 展平、排序、51 点经验 CDF、均值、降幅 | Initial 读取失败：该指标基线不可用；Calibrated 读取失败：整批拒绝。 |
