...
- 小数归一：解析为有限数后，**四舍五入保留 2 位小数**（第 3 位小数 ≥5 则进位）。超过 2 位**不**拒绝，归一后再做范围处理。实现须对绝对值进位再还原符号：`sign(x) * round(|x| * 100) / 100`，避免 JS `Math.round` 对负半值偏向 +∞。
- 响应里的 `heatmap` / `kpi` 数字均为归一后、并经过范围处理后的值。
- 行内逗号或空白都是分隔符；`238.17, 240.67, 235.66, ` 这类空格与末尾逗号产生的空 token 忽略，不视为无效文件。
- 范围默认值按指标区分，可由 `code/server/.env` 的 `CASE2_RANGE_*` 覆盖（`min,max` 或 `min~max`）。非法环境变量启动失败，不静默修正边界。默认：
...
- 形状：忽略首尾空行，中间非空行必须等长；非空矩形即可，允许 `1×1`、`1×N`、`N×1`。格子与地图位置一一对应，**不得删格**。
- 数值范围（归一后）：按该指标热力 `[min,max]`。越界做**双边掐位**（`x>max → max`，`x<min → min`），保留矩形；将该文件掐位个数记为 `invalidCount` 打 `warn` 日志 `case2 data values out of range`。不因越界返回 `DATA_FILE_INVALID`。
