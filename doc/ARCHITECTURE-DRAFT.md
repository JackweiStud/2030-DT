...
> 本文冻结 Gate 0 的责任边界，不是实现规格；已批准接口见 `doc/case2/API-CONTRACT.md` / `doc/case3/API-CONTRACT.md` / `doc/case4/API-CONTRACT.md`，具体端口、目录注入与控制写入算法见各 case Gate 3 SPEC；本地实现与 QA 证据见 `doc/case2/QA-EVIDENCE.md` / `doc/case3/QA-EVIDENCE.md` / `doc/case4/QA-EVIDENCE.md`。

## 项目模式

- 多并列 case 项目，单一 Web 入口，四个顶部 Tab。
- 当前 case2、case3、case4 均已接入正式运行页；仅 case1 仍为 Shell 占位，不读取共享目录。case4 除 3D 外功能已完成本地开发。
- case2、case3、case4 均为独立业务 case，不可因同属 IMT-2030 而互相继承指标、状态机或字段；可复用 Shell、Node 适配骨架和 Gate 节奏。
...
| case4 Web | case4 三方案定位可见状态、2D 地图/误差/吞吐/CDF/CEP/NLOS、2D Reflection、完成态截图触发 | 直接访问共享目录、真开 3D、把反射当完成硬门槛 |
| Node 本地适配服务（前端 PC） | 唯一文件 I/O、锁、控制文件读写、稳定结果读取、case2/case3/case4 截图 PNG 落盘、case3 单侧实时文件清空与多 txt 结构化收编、case4 轨迹/吞吐/最终快照 | 后端业务采集、其他 case 的业务决策 |
| 后端业务进程（后端 PC） | 读取控制、执行 case2 校准、case3 通信测试或 case4 定位测试、发布结果/append 文件、回写 `status` 与必要标志 | 浏览器 UI、前端截图编码、清空 case3 历史文件 |
| 本地模拟后端（打桩） | 在无真实后端时扮演共享目录另一端，推进状态并发布 synthetic/stub Calibrated 文件 | 真实采集、真实算法、REST 接口 |

## 目标数据流

```text
Chrome case2 / case3 / case4
...
case4:
后端 append 三轨迹 / 两吞吐 / 可选反射，关闭统计后再写 case complete
  -> Node GET /api/case4/trajectory、/throughput、/result
  -> Web 展示 2D 地图、XYZ 误差、吞吐、CDF/CEP/NLOS、2D Reflection
  -> 截图保存到 {DT_SHARED_DIR}/out/case4/
```
...
- 单活跃生命周期：切离 case2/case3/case4 时卸载对应 case-local 状态、停止轮询、清理计时器；切回时按当前适配服务状态重新初始化。
...
case4 后端文件层为 `{DT_SHARED_DIR}/case4/` 下的预期轨迹、三路实时定位、两路吞吐、三份 CDF 与一份 CEP/NLOS 汇总；可选反射文件见 [REFLECTION-SPEC.md](case4/REFLECTION-SPEC.md)。Node 提供 `/api/case4/trajectory`、`/throughput`、`/result`；完成门槛只看原九个动态文件加控制上下文，反射尽力附加、不挡 200。Web 只消费 REST 快照。

截至 2026-09-18，Case4 正式 Web、Node 适配服务和本地模拟后端已完成本地开发；除 3D 外功能已交付。该结论覆盖本地打桩与隔离目录联调，不覆盖本仓库对真实后端 PC、真实挂载路径或真实采集数据的独立验收。3D 未实现。

## 不纳入本草案

- 本草案不重复记录 WebSocket、Node 服务端口、REST 资源路径、数据缓存等施工细节；以各 case API 契约和 SPEC 为准。
- 真后端算法、文件格式升级、截图文件命名、共享目录部署脚本。
- case1 的业务复用判断。case4 已按独立 case 完成本地开发，详见 `doc/case4/`。
