...
首版仅 2D，三方案同时展示。百分比、悬停、3D、反射后置。静态页面只提供视觉、样式和组件结构；固定 Pencil 坐标、示意 CDF/CEP、URL 假状态不能进入正式运行路径。
...
- 坐标严格 3 个**逗号**分隔字段。吞吐为单字段。CDF 与 CEP/NLOS 汇总为 2 列：同一文件内空白分隔或逗号分隔二选一，不得混用；列含义见 §2，不因分隔符改变字段。
