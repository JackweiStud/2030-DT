...
| UI 按钮 | **重置** | 清空 case2 本地完成态并提交协议命令 `reinit`。界面不再使用“清除”作为按钮文案。 |
| 协议命令 | `init` / `start` / `reinit` | 初始化/空闲、开始测试、重置。 |
| 后端状态 | `""` / `execute success` / `execute fail` / `case complete` | 初始化、命令已执行但未完成、执行失败、结果已可读取。 |
...
| 提交重置 | Web → 适配服务 → 控制文件 | 用户点击“重置” | 写入 `command=reinit` | 提交后立刻清空本地 Calibrated 结果；回到 Initial 的后端确认规则见 P0-2。 |
...
| `status` | `""` / `execute success` / `execute fail` / `case complete` | 后端 | Web 只读解释，绝不以此字段向后端回写业务结论。 |
...
2. 后端拥有 `status`；Web 或适配服务不得用前端本地状态伪造 `execute success`、`execute fail` 或 `case complete`。
...
适配服务可达时，case2 Web 按以下优先级解释快照。前端本地临时状态不能覆盖后端已给出的失败或完成事实。

| 优先级 | 条件 | case2 可见状态 | Calibrated 区域 |
|---:|---|---|---|
| 1 | 适配服务不可达或快照无效 | 连接/读取异常，不能冒充业务失败 | 不展示旧结果；Initial 保留或显示其自身读取错误。 |
| 2 | `status=case complete` 且六文件批次通过全部校验 | `completed` | 显示新批次热力图、CDF、均值与降幅。 |
| 3 | `status=case complete` 但批次缺失、解析失败或无法证明稳定 | 结果发布异常 | 不显示完成态，也不得回退为旧 Calibrated 结果。 |
| 4 | `status=execute fail` | `failed` | 保留 Initial；清空 Calibrated；允许重新启动或重置。 |
| 5 | `command=start`，或本地启动已被适配服务接受，且尚未命中上述条件 | `calibrating` / `execute-success-waiting` | 不展示任何旧结果。`execute success` 仍属于等待结果。 |
| 6 | `command=init` 且 `status=""` | `initial` | Initial 可见；Calibrated 为空态。 |
| 7 | 本地已提交 `reinit`，但后端初始确认尚未到达 | `resetting` | 立即清空 Calibrated，等待 P0-2 所定义的确认条件。 |
| 8 | 其他组合 | 未知控制状态 | 不得显示完成态；保留诊断信息给适配服务/QA。 |
...
| 重置 | 立即清空本地完成态，提交 `reinit` | 写命令并继续读取快照 | 恢复初始语义 | 等待确认后回 Initial。 |
...
|---|---|---|---|
| 热力图矩阵 | 20×20 误差空间分布 | 文本；CRLF 或 LF；逗号或空白分隔 | 恰好 20 行，每行 20 个有限数值；无缺失、额外非空行或非数值。 |
| KPI 样本 | 20 个误差标量 | 文本；CRLF 或 LF；逗号或空白分隔 | 恰好 20 个有限数值。行列分组不表达业务语义。 |

参考说明曾将 KPI 举例为 4×5，实际十二份 KPI 参考文件均为 20×1。为避免把文件排版误写成接口语义，本文只冻结“每指标每阶段 20 个标量样本”；前端以展平数组计算 CDF 与均值。
...
| UI 输出 | 输入 | 固定算法/规则 |
|---|---|---|
| Initial / Calibrated 热力色场 | 对应 20×20 矩阵 + `04-runtime-assets/case2/maps/heatmap-map-base.png` | 运行时按已提供热力图说明进行插值、配色和马赛克叠加；静态代表图 `heatmap-calibrated-represent.png` 不得进入正式运行路径。 |
| CDF | 每项对应的 20 个 KPI 样本 | 升序经验 CDF；按 `t=0..1` 的 51 个等距位置取 `floor(t×(n-1))`，其中 `n=20`。 |
| 平均误差 | 每项对应的 20 个 KPI 样本 | 算术平均；显示精度由 Gate 3 WEB-SPEC 冻结。 |
...
- 当前 UI 只消费 RSS、有效路径数、首径时延三项；每项包括 20×20 热力矩阵与 20 个 KPI 样本。
...
| P0-2 | `reinit` 完成时后端如何恢复 `command` 与 `status`，以及 Web 在刷新后认定 Initial 的精确条件 | 否则重置可以清屏，却无法可靠结束。 |
| P0-3 | 适配服务的 REST 路由、请求/响应 shape、轮询/超时、锁和挂载路径 | 语义已经冻结，但实现端点尚无事实来源。 |
| P0-4 | 截图输出路径、命名、确认回执与进程重启去重策略 | 否则不能保证“已保存才清零”且可能重复输出。 |

**停止条件：** P0-1 至 P0-4 全部得到用户/后端负责人确认，并在 `doc/API-CONTRACT-REVIEW.md` 标记为已修后，用户批准契约 v1；此前不写 Node、React、共享目录连接或真实状态机。
